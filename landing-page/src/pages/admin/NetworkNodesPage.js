...
const cardStyle = {
  background:
    'linear-gradient(135deg, rgba(26, 23, 48, 0.9) 0%, rgba(15, 14, 23, 0.95) 100%)',
  ...
  return (
    ...
    >
      ...
      <Box>
        ...
        <Typography
          sx={{color: '#E8E6F0', fontSize: '1.4rem', fontWeight: 700}}
        >
          {count}
        </Typography>
      </Box>
  ...
  return (
    ...
    >
      ...
      >
        <Button
          onClick={onClose}
          sx={{color: '#72757E', textTransform: 'none'}}
        >
          Cancel
        </Button>
        <Button
      ...
      <DialogContent sx={{pt: 3}}>
        <Typography sx={{color: '#72757E', fontSize: '0.9rem', mb: 2}}>
          ...
          <strong style={{color: '#E8E6F0'}}>
            {request?.user_name || request?.username || 'this node'}
          </strong>{' '}
          as a regional host. Assign a region name for DNS and certificate
          generation.
        </Typography>
        <TextField
          fullWidth
      ...
      >
        ...
        <Button
          ...
          sx={{
            ...actionButtonStyle,
            background:
              'linear-gradient(135deg, rgba(0, 191, 165, 0.2) 0%, rgba(0, 150, 130, 0.2) 100%)',
            color: '#00BFA5',
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      const challengeData = await apiFetch(
        '/api/social/hierarchy/upgrade-challenge'
      );
      ...
      if (seedBytes.length !== 32) {
        throw new Error(
          'Key must be 64 hex characters (32 bytes Ed25519 seed)'
        );
      }
      ...
      setTimeout(() => {
        showSnackbar(
          'Node is restarting as central. Page will reload shortly.',
          'info'
        );
  ...
  const handleApprove = async (regionName) => {
    ...
    try {
      await apiFetch('/api/social/regional-host/approve', {
        method: 'POST',
        body: JSON.stringify({
          request_id: req.id || req.request_id,
        ...
        <Grow in={!loadingNodes} timeout={400}>
          <Grid container spacing={2} sx={{mb: 3}}>
            ...
            <Grid item xs={6} sm={4} md={2}>
              <StatCard
                label="Regional"
                count={nodeStats.regional}
                color={TIER_COLORS.regional}
                icon={<HubIcon />}
              />
            </Grid>
            <Grid item xs={6} sm={4} md={2}>
              ...
              {tierInfo?.tier === 'central' ? (
                ...
                <Box sx={{display: 'flex', alignItems: 'center', gap: 2}}>
                  ...
                  <Box>
                    ...
                    <Typography
                      sx={{color: '#72757E', fontSize: '0.82rem', mt: 0.3}}
                    >
                      Full network authority active. HART tag:{' '}
                      {tierInfo.hart_tag || '--'}
                    </Typography>
                  </Box>
                </Box>
                ...
                <Box>
                  ...
                  >
                    Paste a master private key to upgrade this node to Central
                    tier. The key is signed locally using Ed25519
                    challenge-response — only the cryptographic signature is
                    sent, never the key itself. Safe from MITM, proxy
                    interception, and source code inspection.
                  </Typography>
                  <Box
                  ...
                  >
                    ...
                    <Button
                      ...
                      sx={{
                        ...
                        height: 40,
                        background:
                          'linear-gradient(135deg, rgba(255, 215, 0, 0.15) 0%, rgba(255, 180, 0, 0.15) 100%)',
                        color: '#FFD700',
                  ...
                  >
                    ...
                    Challenge-response protocol: key signs a one-time nonce
                    locally, only the signature crosses the wire. Even with Burp
                    Suite, MITM, or public source code, the private key cannot
                    be extracted.
                  </Typography>
                </Box>
              )}
                ...
                <Table size="small">
                  ...
                  <TableBody>
                    {nodes.map((node, index) => {
                      ...
                      const hartTag =
                        node.hart_tag || node.name || node.node_id;
                          ...
                          >
                              ...
                              >
                                ...
                                <Tooltip title="RAM (GB)" arrow>
                                  ...
                                  >
                                    ...
                                    >
                                      {node.compute_ram_gb != null
                                        ? `${node.compute_ram_gb}G`
                                        : '--'}
                                    </Typography>
                                  </Box>
                                </Tooltip>
                                {node.compute_gpu_count != null &&
                                  node.compute_gpu_count > 0 && (
                            ...
                            <TableCell>
                              <Typography sx={{fontSize: '0.85rem'}}>
                                <Typography
                                  component="span"
                                  sx={{fontWeight: 600, color: '#E8E6F0'}}
                                >
                                  ...
                                  <>
                                    <Tooltip title="Demote to Flat" arrow>
                                      <Button
                                        size="small"
                                        onClick={() =>
                                          handleDemote(node.node_id, hartTag)
                                        }
                                        disabled={
                                          actionLoading === node.node_id
                                        }
                                    ...
                                    <Tooltip title="Revoke Certificate" arrow>
                                      <IconButton
                                        size="small"
                                        onClick={() =>
                                          handleRevoke(node.node_id, hartTag)
                                        }
                                        disabled={
                                          actionLoading === node.node_id
                                        }
                            ...
                            <TableCell>
                              <Typography
                                sx={{
                                  ...
                                  fontWeight: 600,
                                  color:
                                    (req.trust_score || 0) >= 80
                                      ? '#00BFA5'
                                      : (req.trust_score || 0) >= 50
                                        ? '#FFD700'
                                        : '#FF6B6B',
                                }}
                              >
                                {req.trust_score != null
                                  ? req.trust_score
                                  : '--'}
                              </Typography>
                            </TableCell>
                                ...
                                <Box sx={{display: 'flex', gap: 0.8}}>
                                  <Button
                                    ...
                                    sx={{
                                      ...actionButtonStyle,
                                      background:
                                        'linear-gradient(135deg, rgba(0, 191, 165, 0.15) 0%, rgba(0, 150, 130, 0.15) 100%)',
                                      color: '#00BFA5',
                                      border:
                        ...
                        >
                          ...
                          >
                            ...
                            <Box sx={{mb: 2}}>
                              ...
                              >
                                <Typography
                                  sx={{color: '#72757E', fontSize: '0.75rem'}}
                                >
                                  Utilization
                                </Typography>
                                <Typography
                            ...
                            <Grid container spacing={1}>
                              ...
                              <Grid item xs={6}>
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    >
                                      ({region.active_node_count} active)
                                    </Typography>
                                  </Typography>
                                </Box>
                              </Grid>
                              <Grid item xs={4}>
                                ...
                                >
                                  <Typography
                                    sx={{color: '#72757E', fontSize: '0.68rem'}}
                                  >
                                    Cores
                                  </Typography>
                                  <Typography
