...
const channelCardSx = (selected) => ({
  bgcolor: selected ? 'rgba(108, 99, 255, 0.15)' : 'rgba(26, 26, 46, 0.7)',
  border: selected
...
export default function ChannelSetupWizard({open, onClose, onSuccess}) {
    ...
    try {
      ...
      try {
        ...
        setTestResult({
          ...
          binding,
          warning:
            'Channel added but connection test failed. It may still work.',
        });
      }
    } catch (err) {
      const msg =
        ...
        'Failed to create channel binding';
      setError(msg);
  ...
  const renderAuthFields = () => {
        ...
        <Box sx={{my: 2}}>
          <QRPairingDisplay
            onPaired={(data) => {
              setFormData((prev) => ({
                ...prev,
                session_token: data.token || data.code,
              }));
            }}
          />
          {formData.session_token && (
            <Alert
              severity="success"
              sx={{mt: 2, bgcolor: 'rgba(0,232,157,0.1)', color: '#00e89d'}}
            >
    ...
    if (method === 'oauth2') {
      return (
        <Box sx={{my: 3, textAlign: 'center'}}>
          ...
          >
            You will be redirected to authorize{' '}
          ...
          <Button
            ...
            onClick={() => {
              const authUrl =
                selectedChannel.oauth_url ||
                `/api/social/channels/oauth/${selectedChannel.channel_type}/start`;
    ...
    if (fields.length === 0) {
      return (
        <Typography
          variant="body2"
          sx={{color: 'rgba(255,255,255,0.5)', my: 2}}
        >
            ...
            return (
              <Box key={cat} sx={{mb: 2}}>
                ...
                >
                  ...
                  <Typography
                    variant="overline"
                    sx={{color: 'rgba(255,255,255,0.4)', letterSpacing: 2}}
                  >
                ...
                <Collapse in={isExpanded}>
                  <Grid container spacing={1.5}>
                    {grouped[cat].map((ch) => {
                      const isSelected =
                        selectedChannel?.channel_type === ch.channel_type;
                      return (
  ...
  return (
    ...
    >
      ...
      <DialogActions
        sx={{px: 3, py: 2, borderTop: '1px solid rgba(255,255,255,0.06)'}}
      >
        {step === 2 ? (
          <Button
