...
export default function ConversationHistoryPanel({channelTypes}) {
  ...
  const fetchMessages = useCallback(
    async (pageNum, channelFilter, append) => {
      ...
      try {
        ...
        setHasMore(
          meta.has_next !== undefined ? meta.has_next : items.length >= 20
        );

        // Extract unique channel types for filter tabs
        if (pageNum === 1 && !append) {
          const types = [
            ...new Set(items.map((m) => m.channel_type).filter(Boolean)),
          ];
        ...
        <Fade in>
          <Box sx={{textAlign: 'center', py: 6}}>
            <ChatBubbleOutlineIcon
              sx={{fontSize: 48, color: 'rgba(255,255,255,0.12)', mb: 1}}
            />
              ...
              <Fade in timeout={200 + idx * 50} key={msg.id || idx}>
                ...
                >
                  ...
                  >
                    ...
                    <Typography
                      variant="caption"
                      sx={{color: style.labelColor, fontSize: '0.65rem'}}
                    >
                      {style.label}
                    </Typography>
                    <Typography
                      variant="caption"
                      sx={{color: 'rgba(255,255,255,0.2)', fontSize: '0.6rem'}}
                    >
