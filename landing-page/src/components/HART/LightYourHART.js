    ...
    function draw() {
      ...
      for (const p of particlesRef.current) {
        ...
        if (palette && phase !== 'darkness' && phase !== 'language') {
          const target =
            palette.base + (Math.random() - 0.5) * palette.spread * 0.1;
        ...
        if (isConverging) {
          ...
          p.vx +=
            (-dy / dist) * 0.02 + (((targetDist - dist) * dx) / dist) * 0.001;
          p.vy +=
            (dx / dist) * 0.02 + (((targetDist - dist) * dy) / dist) * 0.001;
  ...
  const preSynth = useCallback(
    async (lineId, text) => {
      try {
        ...
        /* Backend TTS unavailable — speak() falls through to Web Speech */
      }
      return false;
    },
    [language]
  ...
  const warmUp = useCallback(
    async (lang) => {
      ...
      try {
        ...
        /* Backend down — will use pre-synth .ogg + Web Speech fallback */
      }
      return 'unavailable';
    },
    [language]
    ...
    <Box sx={{textAlign: 'center'}}>
      ...
      <Typography
        ...
        sx={{
          ...
          animation:
            lettersVisible > 0
              ? 'hart-at-breathe 4s ease-in-out infinite'
              : 'none',
...
export default function LightYourHART({userId, onComplete}) {
  ...
  const answerPassion = useCallback(
    async (key) => {
      setPassionKey(key);
      setOptions([]);
      advance('answer', {key});
  ...
  const answerEscape = useCallback(
    async (key) => {
      setEscapeKey(key);
      setOptions([]);
      advance('answer', {key});
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      await _sleep(3000);
      if (onComplete)
        onComplete({name: hartName, hartTag, emojiCombo, language, locale});
    })();
    ...
    <>
      ...
      <Box sx={styles.root}>
        ...
        <Box sx={styles.content}>
          ...
          {['greeting', 'ack_passion', 'ack_escape', 'pre_reveal'].includes(
            phase
          ) &&
            paText && (
