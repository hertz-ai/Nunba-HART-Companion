...
const _particles = Array.from({length: 40}, () => ({
  ...
  y: Math.random() * 100,
  d: 12 + Math.random() * 20,
  dl: -(12 + Math.random() * 20) * Math.random(),
...
const HeroContent = ({
  ...
  <>
    {hartName && (
      <p style={hartGreetingStyle(mobile)}>Hey @{hartName}, meet your agent</p>
    )}
    ...
    <button
      className="inline-flex items-center justify-center px-8 py-4 text-lg font-medium rounded-full transition-all"
      style={{
...
const AgentPage = () => {
  ...
  const hartName = useMemo(
    () => localStorage.getItem('hart_name') || '',
    [hartSealed]
  ); // eslint-disable-line react-hooks/exhaustive-deps
  const hartEmoji = useMemo(
    () => localStorage.getItem('hart_emoji') || '',
    [hartSealed]
  ); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    ...
    const t1 = setTimeout(async () => {
      ...
      setWelcomePhase(1);
      const lang =
        hartLanguage || localStorage.getItem('hart_language') || 'en';
      try {
  ...
  useEffect(() => {
    const eU = localStorage.getItem('user_id');
    const eE = localStorage.getItem('email_address');
    if (eU && eE) {
      setDecryptedUserId(decrypt(eU));
      setDecryptedEmail(decrypt(eE));
    }
  }, []);
    ...
    (async () => {
      try {
        ...
        try {
          ...
          allAgents = Array.isArray(arr)
            ? arr.map((a) => ({...a, _isLocal: a.type === 'local'}))
            : [];
          logger.log('Agent.js: Fetched local agents:', allAgents.length);
        } catch (e) {
          console.warn('Agent.js: Local backend not available:', e.message);
        }
        setAgentData(
          allAgents.find(
  ...
  if (!hartSealed) {
    return (
      <LightYourHART userId={effectiveUserId} onComplete={handleHartComplete} />
    );
  }
  ...
  if (showWelcome && !welcomeDone) {
    ...
    return (
      <div
        ...
        style={{background: '#0F0E17', overflow: 'hidden'}}
      >
        {_particles.slice(0, 20).map((p, i) => (
          <div
            ...
            style={{
              ...
              opacity: p.o,
              animation: `welcomeDot ${p.d}s ease-in-out ${p.dl}s infinite`,
              pointerEvents: 'none',
        ...
        >
          ...
          {hartName && (
            <div
              style={{
                ...
                marginBottom: 16,
                opacity: welcomePhase >= 2 ? 0.4 : 1,
                transition: 'opacity 0.8s ease',
              }}
            >
              {hartEmoji && (
                <span style={{marginRight: 12, fontSize: '0.8em'}}>
                  {hartEmoji}
                </span>
              )}
              @{hartName}
          ...
          <div
            style={{
              ...
              animation: 'welcomeTextIn 1.5s ease-out 0.5s both',
              opacity: welcomePhase >= 1 ? 0 : 1,
          ...
          {welcomePhase >= 1 && (
            <div style={{animation: 'welcomeTextIn 0.8s ease-out both'}}>
              ...
              {/* GPU info line */}
              {bootstrapStatus?.gpu_name &&
                bootstrapStatus.gpu_name !== 'CPU only' && (
                  <div
                    style={{
                  ...
                  >
                    {bootstrapStatus.gpu_name} &middot;{' '}
                    {bootstrapStatus.vram_total_gb}GB VRAM
                  </div>
                )}
          ...
          {welcomePhase >= 2 && (
            <div
              style={{
                marginTop: 20,
                animation: 'welcomeTextIn 0.6s ease-out both',
                color: '#6C63FF',
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-black gap-3">
        <div
          className="text-red-400 text-sm px-4 py-2 rounded-xl"
          style={{
    ...
    <div className="relative h-screen">
      ...
      >
        ...
        {_particles.map((p, i) => (
          <div
            ...
            style={{
              ...
              opacity: heroEntrance ? p.o : 0,
              transition: `opacity 1.5s ease ${i * 0.03}s`,
              animation: heroEntrance
                ? `particleFloat ${p.d}s ease-in-out ${p.dl}s infinite`
                : 'none',
              pointerEvents: 'none',
              zIndex: 0,
            }}
        ...
        <div
          ...
          style={entranceStyle(heroEntrance)}
        >
          <div className="h-[35vh] flex items-center justify-center relative">
            <img
              src={heroImg}
              alt="hero"
              className="absolute top-0 object-cover rounded-lg"
              style={{
        ...
        <div
          ...
          style={entranceStyle(heroEntrance)}
        >
          <h1 className="text-2xl font-semibold text-white ml-4 mt-4">
            HevolveAI
          </h1>
          <div className="relative h-full flex items-center justify-center flex-col text-center">
            ...
            <img
              src={heroImg}
              alt="hero"
              className="absolute bottom-44 right-5 object-cover rounded-lg"
              style={{
                width: getVideoWidthforMobile(),
                animation: 'heroFloat 4s ease-in-out infinite',
              }}
      ...
      <div
        ...
        style={{
          ...
          filter: heroVisible ? 'blur(4px)' : 'blur(0px)',
