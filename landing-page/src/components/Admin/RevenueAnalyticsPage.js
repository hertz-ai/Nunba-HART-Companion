  ...
  return (
    <Grow in={true} timeout={300 + delay}>
      ...
      >
        ...
        {subtitle && (
          <Typography
            sx={{color: 'rgba(255,255,255,0.4)', fontSize: '0.8rem', mt: 0.5}}
          >
            {subtitle}
          </Typography>
        )}
...
export default function RevenueAnalyticsPage() {
  ...
  const sortedUsers = useMemo(() => {
    ...
    return [...data.per_user].sort((a, b) => {
      const aVal =
        typeof a[sortField] === 'string' ? a[sortField] : a[sortField] || 0;
      const bVal =
        typeof b[sortField] === 'string' ? b[sortField] : b[sortField] || 0;
              ...
              <Box sx={{display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap'}}>
                <Grow in={true} timeout={500}>
                  <Box sx={{...cardStyle, flex: 1, minWidth: 400, p: 3}}>
                    ...
                    <Box sx={{height: 300}}>
                      {timeSeries.ad_daily?.length ||
                      timeSeries.spark_daily?.length ? (
                        <Line
                          data={buildRevenueChartData(
                            timeSeries.ad_daily || [],
                            timeSeries.spark_daily || []
                          )}
                      ...
                      {sortedUsers.map((u, i) => (
                        <Fade in={true} timeout={300 + i * 30} key={u.user_id}>
                          ...
                          >
                            ...
                            <TableCell>
                              <Typography
                                sx={{color: '#6C63FF', fontWeight: 600}}
                              >
                                {u.ad_revenue}{' '}
                                <Typography
                                  component="span"
                            ...
                            <TableCell>
                              <Typography
                                sx={{color: '#9B94FF', fontWeight: 600}}
                              >
                                {u.compute_cost.toFixed(2)}{' '}
                                <Typography
                                  component="span"
                                  sx={{
                                    color: 'rgba(255,255,255,0.3)',
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            ...
                            <Typography
                              sx={{color: '#fff', fontWeight: 500, flex: 1}}
                            >
                              {owner.username}
                            </Typography>
                            <Chip
                              size="small"
                              label={`${owner.owned_agents.length} owned`}
                              sx={{
                            ...
                            {owner.external_agents_used.length > 0 && (
                              <Chip
                                size="small"
                                label={`${owner.external_agents_used.length} external`}
                                sx={{
                                ...
                                {owner.external_agents_used.map((ext) => (
                                  <Chip
                                    ...
                                    sx={{
                                      ...
                                      color: '#9B94FF',
                                      border:
                                        '1px solid rgba(155, 148, 255, 0.2)',
                                      fontWeight: 500,
                                    }}
                                  />
                                ))}
