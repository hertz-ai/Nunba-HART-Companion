...
const DISCOVERABLE_FEATURES = [
  {
    key: 'encounters',
    label: 'Encounters',
    desc: 'Meet people and ideas nearby',
  ...
  },
  {
    ...
    desc: 'Explore community ideas and hypotheses',
  ...
  },
  {
    key: 'games',
    label: 'Games',
    desc: 'Play games and compete with friends',
    path: '/social/games',
    icon: 'SportsEsports',
  },
  {
    key: 'kids',
    label: 'Kids Learning',
    desc: 'Fun learning games for young minds',
    path: '/social/kids',
    icon: 'SportsEsports',
  },
  {
    key: 'recipes',
    label: 'Recipes',
    desc: 'Browse community-created AI agent recipes',
    path: '/social/recipes',
    icon: 'AutoAwesome',
  },
  {
  ...
  },
  {
    key: 'campaigns',
    label: 'Campaigns',
    desc: 'Create and join community campaigns',
...
function FeatureSpotlightCard({onDismiss}) {
  ...
  useEffect(() => {
    try {
      const visited = JSON.parse(
        localStorage.getItem('visited_features') || '[]'
      );
      const undiscovered = DISCOVERABLE_FEATURES.filter(
        (f) => !visited.includes(f.key)
      );
      ...
      setFeature(
        DISCOVERABLE_FEATURES[
          Math.floor(Math.random() * DISCOVERABLE_FEATURES.length)
        ]
      );
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      <Typography
        variant="body2"
        sx={{color: 'rgba(255,255,255,0.7)', mb: 1.5, lineHeight: 1.5}}
      >
        {feature.desc}
      </Typography>
      <Button
...
export default function FeedPage() {
  ...
  useEffect(() => {
    seasonsApi
      .current()
      .then((res) => {
        if (res && res.data) setSeason(res.data);
      })
      .catch(() => {});
    encountersApi
      .suggestions()
      .then((res) => {
        if (res && res.data) setEncounters(res.data);
      })
      .catch(() => {});
  }, []);
  ...
  const fetchPosts = useCallback(
    async (reset = false) => {
      ...
      try {
        ...
        const fetcher =
          feedKey === 'personalized' && !currentUser
            ? fetchers.all
            : fetchers[feedKey];
  ...
  useEffect(() => {
    window.scrollTo({
      top: 0,
      behavior: window.pywebview ? 'instant' : 'smooth',
    });
    setPosts([]);
    setOffset(0);
    setHasMore(true);
  ...
  return (
    <>
      {season && !seasonDismissed && (
        <SeasonBanner
          season={season}
          onDismiss={() => setSeasonDismissed(true)}
        />
      )}
      ...
      >
        <Chip
          ...
          sx={{
            ...
            fontSize: '0.72rem',
            background:
              intentFilter === null ? GRADIENTS.primary : 'transparent',
            color:
              intentFilter === null ? '#fff' : theme.palette.text.secondary,
          ...
          <Box sx={{p: 2.5}}>
            ...
            {hartSpotlight.hart_of_day && (
              <Box
                ...
                onClick={() =>
                  navigate(`/social/agents/${hartSpotlight.hart_of_day.id}`)
                }
              >
                ...
                <Box sx={{flex: 1}}>
                  ...
                  <Typography variant="caption" sx={{opacity: 0.85}}>
                    {hartSpotlight.hart_of_day.description ||
                      'Top performing HART agent today'}
                  </Typography>
                </Box>
                <Chip
                  label={`${hartSpotlight.hart_of_day.score || 0} HARTs`}
                  size="small"
                  sx={{
                    bgcolor: 'rgba(255,255,255,0.2)',
                    color: '#fff',
                ...
                <React.Fragment key={p.id}>
                  {p.is_thought_experiment ? (
                    <ThoughtExperimentCard
                      post={p}
                      animationDelay={Math.min(idx * 50, 400)}
                      index={idx}
                    />
                  ) : (
                    <PostCard
      ...
      {canWrite && (
        <Fab
          ...
          aria-label="Create thought experiment"
        >
          <AddIcon />
        </Fab>
      )}
