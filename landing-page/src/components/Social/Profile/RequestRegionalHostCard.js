...
export default function RequestRegionalHostCard({userId}) {
  ...
  const fetchEligibility = useCallback(async () => {
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/social/regional-host/eligibility`,
        {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/social/regional-host/request`,
        {
          method: 'POST',
          headers: getAuthHeaders(),
          body: JSON.stringify({
  ...
  const hasActiveRequest =
    existing_request &&
    ['pending', 'pending_steward', 'approved'].includes(
      existing_request.status
    );
  const isApproved = existing_request?.status === 'approved';
  const isPending =
    existing_request &&
    ['pending', 'pending_steward'].includes(existing_request.status);
  ...
  return (
    <Box sx={cardStyle}>
      ...
      >
        ...
        <Typography
          sx={{color: '#E8E6F0', fontWeight: 600, fontSize: '0.95rem', flex: 1}}
        >
        ...
        {!isApproved && (
          ...
          >
            ...
            <Box>
              ...
              <Typography
                sx={{color: '#72757E', fontSize: '0.75rem', lineHeight: 1.5}}
              >
                2x karma multiplier &bull; Priority hive compute allocation
                &bull; Steward recognition badge &bull; Regional DNS namespace
                &bull; Certificate authority for your region
              </Typography>
            </Box>
          </Box>
        )}
          ...
          <Box sx={{display: 'flex', alignItems: 'center', gap: 1.5, mb: 1.2}}>
            ...
            <Box sx={{flex: 1}}>
              <Box sx={{display: 'flex', alignItems: 'center', gap: 1}}>
                ...
                <Typography
                  sx={{color: '#E8E6F0', fontSize: '0.85rem', fontWeight: 500}}
                >
                  Compute Tier
                </Typography>
              </Box>
              <Typography sx={{color: '#72757E', fontSize: '0.75rem', mt: 0.2}}>
                ...
                <span
                  style={{
                    color: tierColors[compute_tier] || '#72757E',
                    fontWeight: 600,
                  }}
                >
                  {compute_tier}
                ...
                &mdash; Minimum:{' '}
                <span style={{color: '#00BFA5', fontWeight: 600}}>
                  STANDARD
                </span>{' '}
                (
                {requirements?.compute_description?.STANDARD ||
                  '4+ cores, 8+ GB RAM'}
                )
              </Typography>
            </Box>
        ...
        {isApproved ? (
          ...
          >
            ...
            <Typography sx={{color: '#E8E6F0', fontSize: '0.85rem'}}>
              You are an active regional host for{' '}
              <strong style={{color: '#6C63FF'}}>
                {existing_request?.region_name || 'your region'}
              </strong>
            </Typography>
          </Box>
        ) : isPending ? (
          ...
          >
            ...
            <Typography sx={{color: '#E8E6F0', fontSize: '0.85rem'}}>
              Your request is awaiting steward approval. You'll be notified when
              reviewed.
            </Typography>
          </Box>
        ) : (
          <>
            ...
            {result?.error && (
              <Alert
                severity="error"
                sx={{mb: 1.5, borderRadius: '8px', fontSize: '0.82rem'}}
              >
                {result.error}
              </Alert>
            )}
                  ...
                  <Box sx={{p: 0.5}}>
                    ...
                    {!meets_compute && (
                      <Typography sx={{fontSize: '0.75rem'}}>
                        - Compute tier must be STANDARD or higher (current:{' '}
                        {compute_tier})
                      </Typography>
                    )}
                    {!meets_trust && (
                      <Typography sx={{fontSize: '0.75rem'}}>
                        - Trust score must be at least 2.5 (current:{' '}
                        {trust_score})
                      </Typography>
                    )}
                  </Box>
      ...
      >
        ...
        <DialogContent sx={{pt: 3}}>
          <Typography sx={{color: '#72757E', fontSize: '0.9rem', mb: 2}}>
            Your request will be reviewed by a network steward. Upon approval,
            you'll receive a regional certificate and DNS namespace.
          </Typography>
          <TextField
            fullWidth
            size="small"
