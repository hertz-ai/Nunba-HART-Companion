...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    resonanceApi
      .getWalletFor(userId)
      .then((res) => {
        if (!cancelled && res && res.data) setWallet(res.data);
      })
      .catch(() => {})
      .finally(() => {
        ...
        >
          ...
          <Box sx={{flex: 1, minWidth: 0}}>
            ...
            <Box sx={{display: 'flex', alignItems: 'center', gap: 0.5}}>
              ...
              <Tooltip title="User RSS feed">
                <IconButton
                  size="small"
                  onClick={() =>
                    window.open(feedsApi.getUserRssUrl(user.id), '_blank')
                  }
                  sx={{
            ...
            {walletLoading ? (
              ...
              wallet && (
                <Box sx={{mt: 1.5}}>
                  <ResonanceWallet wallet={wallet} compact />
                </Box>
              )
            )}
            {user && (
              <Box sx={{mt: 1.5}}>
                <RatingsPanel userId={user.id} isOwnProfile={isOwn} />
              </Box>
            )}
            {isOwn && (
              <Box sx={{mt: 1.5}}>
            ...
            {isOwn && (
              <Box sx={{mt: 1.5}}>
                <RequestRegionalHostCard userId={user.id} />
              </Box>
            )}
          </Box>
        </Box>
