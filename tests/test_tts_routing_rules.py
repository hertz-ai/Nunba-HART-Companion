...
class TestInternationalRouting:
    ...
    def test_all_intl_prefer_cosyvoice3(self):
        for lang in self.INTL_LANGS:
            prefs = _FALLBACK_LANG_ENGINE_PREFERENCE[lang]
            assert prefs[0] == BACKEND_COSYVOICE3, \
                f"Lang {lang} should prefer CosyVoice3, got {prefs[0]}"

    def test_intl_has_chatterbox_ml_fallback(self):
        for lang in self.INTL_LANGS:
            prefs = _FALLBACK_LANG_ENGINE_PREFERENCE[lang]
            assert BACKEND_CHATTERBOX_ML in prefs, f"Lang {lang} missing Chatterbox ML fallback"

    def test_japanese_prefers_cosyvoice3(self):
        assert _FALLBACK_LANG_ENGINE_PREFERENCE['ja'][0] == BACKEND_COSYVOICE3

    def test_chinese_prefers_cosyvoice3(self):
        assert _FALLBACK_LANG_ENGINE_PREFERENCE['zh'][0] == BACKEND_COSYVOICE3
...
class TestDefaultFallback:
    ...
    def test_default_starts_with_cosyvoice3(self):
